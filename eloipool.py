...
def checkShare(share):
	...
	if gotwork and blkhashn <= config.GotWorkTarget:
		try:
			...
			info['difficulty'] = share['difficulty']
			info['remoteHost'] = share['remoteHost']
#			print(share['remoteHost'])
			info['rejectReason'] = ''
			info['hash'] = b2a_hex(blkhash).decode('ascii')
#			print(b2a_hex(blkhash))
...
def logShare(share):
	if '_origdata' in share:
		...
		try:
			share['solution_safe'] = b2a_hex(share['blkhash']).decode('utf8')
		except:
			pass
	users = share['username'].split(".")
	share['username_safe'] = 'invalid'
	try:
		if users[0].startswith('V'):
			share['username_safe'] = users[0]
		if users[1].startswith('V'):
			share['username_safe'] = users[1]
		if users[2].startswith('V'):
			share['username_safe'] = users[2]
	except IndexError:
		pass
